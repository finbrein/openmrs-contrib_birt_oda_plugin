...
public class StyledCCombo extends Composite
{
	 ...
	 * @see Widget#getStyle()
	 */
	public StyledCCombo( Composite parent, int style )
	{
		...
		if ( ( style & SWT.FLAT ) != 0 )
			textStyle |= SWT.FLAT;
		text = new StyledText( this, textStyle );
		...
		listener = new Listener( ) {
			...
			public void handleEvent( Event event )
			{
				...
				if ( StyledCCombo.this == event.widget )
				{
					comboEvent( event );
					return;
				}
		...
		filter = new Listener( ) {
			...
			public void handleEvent( Event event )
			{
				...
				if ( shell == StyledCCombo.this.getShell( ) )
				{
					handleFocus( SWT.FocusOut );
				}
			}
	...
	public Point computeSize( int wHint, int hHint, boolean changed )
	{
		...
		width = Math.max( textWidth
				+ 2 * spacer + arrowSize.x + 2 * borderWidth, listSize.x );
	...
	void dropDown( boolean drop )
	{
		...
		itemCount = ( itemCount == 0 ) ? visibleItemCount
				: Math.min( visibleItemCount, itemCount );
		...
		switch ( type )
		{
			...
			case SWT.FocusOut :
			{
				...
				if ( focusControl == arrow
						|| focusControl == list || focusControl == text )
					return;
				hasFocus = false;
				Shell shell = getShell( );
				shell.removeListener( SWT.Deactivate, listener );
	...
	void initAccessible( )
	{
		...
		arrow.getAccessible( ).addAccessibleListener( new AccessibleAdapter( ) {
			...
			{
				e.result = isDropped( )
						? SWT.getMessage( "SWT_Close" ) : SWT.getMessage( "SWT_Open" ); //$NON-NLS-1$ //$NON-NLS-2$
			}
	...
	public boolean isFocusControl( )
	{
		...
		if ( text.isFocusControl( )
				|| arrow.isFocusControl( ) || list.isFocusControl( )
	...
	public void setEnabled( boolean enabled )
	{
		...
		if ( text != null )
		{
			if ( !enabled )
			{
				text.setForeground( Display.getDefault( )
	...
	void textEvent( Event event )
	{
		switch ( event.type )
		{
			...
			case SWT.MouseWheel :
			{
				Event keyEvent = new Event( );
				keyEvent.time = event.time;
				keyEvent.keyCode = event.count > 0 ? SWT.ARROW_UP
						: SWT.ARROW_DOWN;
				keyEvent.stateMask = event.stateMask;
				notifyListeners( SWT.KeyDown, keyEvent );
				if ( isDisposed( ) )
